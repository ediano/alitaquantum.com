...
export const ExchangeLayout = () => {
  ...
  return (
    <>
      <MetaSEO
        title={`Troca de criptomoeda com taxas mais baixas e sem registro - ${dataFlow.fromName?.toUpperCase()} para ${dataFlow.toName?.toUpperCase()}`}
        pathUrl={
          Number(dataFlow.fromAmount) >= Number(dataFlow.minAmount) &&
          dataFlow.fromName &&
          dataFlow.toName
            ? `/trocar?fromAmount=${dataFlow.fromAmount}&fromName=${dataFlow.fromName}&toName=${dataFlow.toName}`
            : '/trocar'
        }
      />
